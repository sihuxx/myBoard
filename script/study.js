...
window.handleFileSelect = async function(files) {
  for (const file of files) {
    ...
    const path=`study/${Date.now()}_${file.name}`;
    const task=uploadBytesResumable(ref(storage,path),file);
    await new Promise((resolve,reject)=>{
      task.on("state_changed",
        snap=>{ prog.textContent=`업로드 중... ${Math.round(snap.bytesTransferred/snap.totalBytes*100)}%`; },
        err=>{ showToast("업로드 실패: "+err.message); reject(err); },
        async ()=>{
...
function addImgPreview(url, path) {
  ...
  item.innerHTML=`<img src="${url}" alt=""><button class="remove-img" onclick="removeImage('${path}')">✕</button>`;
  list.appendChild(item);
}

window.removeImage = async function(path) {
  try {
...
async function loadPosts() {
  try {
    ...
    document.getElementById("postList").innerHTML=`<div class="state-msg">🔥 Firebase 설정을 확인해주세요.<br><small style="color:#ff4444">${e.message}</small></div>`;
  }
}
...
window.submitPost = async function() {
  ...
  try {
    ...
    if(editMode&&currentPost) { await updateDoc(doc(db,"study_posts",currentPost.id),data); showToast("수정되었습니다."); }
    else { await addDoc(postsCol,{...data,createdAt:serverTimestamp(),uid:currentUser.uid,userEmail:currentUser.email}); showToast("작성되었습니다."); }
    closeModal(); await loadPosts();
  } catch(e){ showToast("오류: "+e.message); }
  finally { btn.disabled=false; btn.textContent=editMode?"수정":"작성"; }
};
...
window.deletePost = async function() {
  ...
  try {
    for(const img of (currentPost.images||[])) { try{await deleteObject(ref(storage,img.path));}catch{} }
